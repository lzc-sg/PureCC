...
def parse_args(input_args=None):
    ...
    parser.add_argument("--eta", type=float, default=2.0, help="The weight of the incremental prior loss (pri_loss).")
    parser.add_argument("--fix_lambda", type=float, default=1.0, help="The scale of the classifier-free guidance increment (ref_model_pred - ref_model_pred_neg).")
    parser.add_argument(
        "--stage1_output_dir",
        type=str,
        default=None,
        required=True,
        help=(
            "Path to the stage1 output directory that contains `learned_embeds.pt`. "
            "The learned concept token embeddings will be loaded from this checkpoint "
            "and registered into the tokenizers/text encoders (frozen, not trained in stage2)."
        ),
    )
    parser.add_argument(
        "--num_class_images",
...
def main(args):
    ...
    ref_transformer = PeftModel.from_pretrained(ref_transformer, f'{args.output_dir}/base_ckpt/{args.csv_name.split(".")[0]}/checkpoint-400')
    image_processor = VaeImageProcessor(vae_scale_factor=vae.config.scaling_factor)

    # ------------------------------------------------------------------ #
    # Load stage1 learned concept token embeddings (frozen in stage2)     #
    # ------------------------------------------------------------------ #
    embeds_path = os.path.join(args.stage1_output_dir, "learned_embeds.pt")
    if not os.path.isfile(embeds_path):
        raise ValueError(
            f"Could not find `learned_embeds.pt` at {embeds_path}. "
            "Please make sure --stage1_output_dir points to the stage1 output folder."
        )
    saved_embeds = torch.load(embeds_path, map_location="cpu")
    new_concept_token = saved_embeds["new_concept_token"]  # e.g. "<new1>"
    logger.info(f"Loading stage1 concept token '{new_concept_token}' from {embeds_path} (frozen in stage2).")

    # Register the new token into tokenizer_one (CLIP-L) and inject the saved embedding.
    num_added_one = tokenizer_one.add_tokens([new_concept_token])
    if num_added_one == 0:
        logger.warning(f"Token '{new_concept_token}' was already present in tokenizer_one; overwriting its embedding.")
    ...
    with torch.no_grad():
        text_encoder_one.get_input_embeddings().weight[new_token_id_one] = (
            saved_embeds["embedding_one"].to(text_encoder_one.dtype)
        )

    # Register the new token into tokenizer_two (CLIP-G) and inject the saved embedding.
    num_added_two = tokenizer_two.add_tokens([new_concept_token])
    if num_added_two == 0:
        logger.warning(f"Token '{new_concept_token}' was already present in tokenizer_two; overwriting its embedding.")
    ...
    with torch.no_grad():
        text_encoder_two.get_input_embeddings().weight[new_token_id_two] = (
            saved_embeds["embedding_two"].to(text_encoder_two.dtype)
        )

    logger.info(
        f"Stage1 concept token '{new_concept_token}' registered: "
        f"id_one={new_token_id_one}, id_two={new_token_id_two}. Embeddings are frozen."
    )
    # ------------------------------------------------------------------ #
    ...
    # transformer.add_adapter("default", transformer_lora_config)
        ...
        if accelerator.is_main_process:
            ...
            for model in models:
                if isinstance(unwrap_model(model), type(unwrap_model(transformer))):
                    ...
                    # model_copy.delete_adapter("dog5")
    ...
    #     if "default" in name:  # 只训练default adapter的参数
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            with accelerator.accumulate(models_to_accumulate):
                ...
                # base_prompt embeds
                base_prompt_embeds, base_pooled_prompt_embeds = compute_text_embeddings(
                    base_prompts, text_encoders, tokenizers
                )

                train_prompt_embeds = torch.cat([prompt_embeds, base_prompt_embeds], dim=0)
                train_pooled_prompt_embeds = torch.cat([pooled_prompt_embeds, base_pooled_prompt_embeds], dim=0)
                ...
                train_model_input = torch.cat([model_input, model_input], dim=0)
                ...
                # zt = (1 - texp) * x + texp * z1
                sigmas = get_sigmas(timesteps, n_dim=train_model_input.ndim, dtype=train_model_input.dtype)
                noisy_model_input = (1.0 - sigmas) * train_model_input + sigmas * noise
                ...
                model_pred = transformer(
                    ...
                    timestep=timesteps,
                    encoder_hidden_states=train_prompt_embeds,
                    pooled_projections=train_pooled_prompt_embeds,
                    return_dict=False,
                )[0]

                model_pred, base_model_pred = torch.chunk(model_pred, 2, dim=0)

                # 两个pred正交约束
                # dot = torch.einsum('bchw,bchw->b', model_pred, base_model_pred)
                # model_pred_norm = torch.sum(model_pred ** 2, dim=1, keepdim=True)**0.5
                # base_model_pred_norm = torch.sum(base_model_pred ** 2, dim=1, keepdim=True)**0.5

                # ortho_loss = torch.mean(dot / (model_pred_norm * base_model_pred_norm)) ** 2

                ref_prompt = batch["target_prompts"] + [""] * len(batch["target_prompts"])
                ref_prompt_embeds, ref_pooled_prompt_embeds = compute_text_embeddings(
                    ref_prompt, text_encoders, tokenizers
                )

                with torch.no_grad():
                    # transformer.module.set_adapter("dog5")
                    with ref_transformer.disable_adapter():
                        ref_model_pred = ref_transformer(
                            hidden_states=noisy_model_input,
                            timestep=timesteps,
                            encoder_hidden_states=ref_prompt_embeds,
                            pooled_projections=ref_pooled_prompt_embeds,
                            return_dict=False,
                        )[0]
                    ref_model_pred, ref_model_pred_neg = torch.chunk(ref_model_pred, 2, dim=0)

                # Follow: Section 5 of https://huggingface.co/papers/2206.00364.
                # Preconditioning of the model outputs.
                if args.precondition_outputs:
                    model_pred = model_pred * (-sigmas.chunk(2)[0]) + noisy_model_input.chunk(2)[0]

                # these weighting schemes use a uniform timestep sampling
                # and instead post-weight the loss
                weighting = compute_loss_weighting_for_sd3(weighting_scheme=args.weighting_scheme, sigmas=sigmas).chunk(2, dim=0)[0]

                # flow matching loss
                if args.precondition_outputs:
                    target = model_input
                else:
                    target = noise.chunk(2, dim=0)[0] - model_input
                ...
                incremental_pred = base_model_pred + args.fix_lambda * (ref_model_pred - ref_model_pred_neg)
                pri_loss = torch.mean(
                    (weighting.float() * (model_pred.float() - incremental_pred.float()) ** 2).reshape(ref_model_pred.shape[0], -1),
                    1,
                )
                pri_loss = pri_loss.mean()
                diff_loss = diff_loss.mean()

                loss = diff_loss + args.eta * pri_loss
            ...
            logs = {"loss": loss.detach().item(), "pri_loss": pri_loss.detach().item() ,"lr": lr_scheduler.get_last_lr()[0]}
